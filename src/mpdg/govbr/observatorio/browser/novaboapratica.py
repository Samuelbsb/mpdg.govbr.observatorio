...
from mpdg.govbr.observatorio.browser.utils import transform_message , get_observatorio_config
...
class NovaBoaPratica(ProcessFiles, form.SchemaForm):
    ...
    def update(self):
        self.createBoaPraticaStructure()  # Cria toda a hieraquia das Boas Praticas
    ...
    def handleApply(self, action):
        ...
        # // Remoção de obrigatoriedade do anexo // 
        ...
        nome_pratica = boapratica.Title()
        descricao_pratica = boapratica.Description()
        url_pratica =boapratica.absolute_url()

        portal_site = api.portal.get()
        text = get_observatorio_config('enviar_email_adm')
        text_obs = transform_message(
            text=text,
        ...
        self.send_email_admin(boapratica.absolute_url(), site_name, text_obs)
    ...
    def send_email_admin(self, link ,title ,menssagem):
        ...
        return simple_send_mail(menssagem,address,subject)

    def get_adm_observatorio(self):
        """Pega o usuário administrador do fale conosco."""
        registry = getUtility(IRegistry)
        adm_observatorio = get_observatorio_config('adm_observatorio')
