...
def get_observatorio_config(config_name):
    registry = getUtility(IRegistry)
    value_config = 'mpdg.govbr.observatorio.controlpanel.IObservatorio_email.{0}'.format(config_name)
    return registry.records[value_config].value


def transform_message(text, **kwargs):
    """Recebe um texto e variados parâmetros. Retorna o texto com as variáveis
    estáticas aceitas substituídas por seus respectivos valores.
    Ex.:
    >>> text = 'O nome da prática cadastrada é [nome_pratica]'
    >>> result = transform_message(text, nome_pratica='Minha prática')
    >>> print result
    >>> 'O nome da prática cadastrada é Minha prática'
    """
    for key in kwargs.keys():
        textvar = '[{}]'.format(key)
        text = text.replace(textvar, kwargs[key])
    return text


def fix_sitesrelacionados_dict(sitesrelacionados):
    """workaround used to fix a bug on sitesrelacionados field at AT BoaPratica when
